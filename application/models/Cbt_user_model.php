...
class Cbt_user_model extends CI_Model{
    ...
    function check_user_password($username, $password){
        $query = $this->get_by_username($username);
        if($query){
            $this->load->library('encryption');
            ...
            if ($password === $decrypted) {
                // Migrate to encryption dynamically if needed
                if ($query->user_password === $decrypted) {
                    $encrypted = $this->encryption->encrypt($password);
                    $this->update('user_name', $username, array('user_password' => $encrypted));
                }
                return true;
            }
        }
