	...
	function login($username, $password){
		...
		if($result){
			...
			if(password_verify($password, $result->password)){
				$is_valid = true;
			}elseif(sha1($password) === $result->password){
				$is_valid = true;
				// Migrate SHA-1 hash to bcrypt
				$this->users_model->update(array('password' => password_hash($password, PASSWORD_DEFAULT)), $username);
			}
