...
class Peserta_daftar extends Member_Controller {
    ...
    function tambah(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            $data['user_password'] = $this->encryption->encrypt($this->input->post('tambah-password', true));
    ...
    function get_by_id($id=null){
    	$data['data'] = 0;
		if(!empty($id)){
			$query_res = $this->cbt_user_model->get_by_kolom('user_id', $id);
			if($query_res->num_rows()>0){
				$query = $query_res->row();
				$this->load->library('encryption');
				$decrypted = $this->encryption->decrypt($query->user_password);
				if($decrypted === FALSE){
					$decrypted = $query->user_password;
				}
				$data['data'] = 1;
    ...
    function edit(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($pilihan=='hapus'){//hapus
                ...
                $password = $this->input->post('edit-password', true);
